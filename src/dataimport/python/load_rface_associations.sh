...
python2.6 db_util.py $config_file
echo begin processing $(date) $dataset_label $feature_matrix $associations_file
python2.6 createSchemaFromTemplate.py $dataset_label ../sql/create_schema_re_template.sql $config_file
echo begin parsing and loading features and sample values $(date) for $feature_matrix_file
python2.6 parse_features_rfex.py $feature_matrix_file $dataset_label $config_file
echo Processing pairwise associations - prepare schema for $dataset_label
python2.6 createSchemaFromTemplate.py $dataset_label ../sql/create_schema_rface_template.sql $config_file
